...
>
> 本文的工程事实以当前 MiniMind `dataset/lm_dataset.py`、`trainer/train_agent.py` 和 LifeOS-Agent 代码为准；论文概念以文末原始论文为准。文中 `P=80`、四条 reward 等数值是为了手算而设置的**教学示例**，不是伪装成训练日志的实测值。
...
```text
P=80, R1=38, O=16, R2=9
总长 L = 80 + 38 + 16 + 9 = 143
```

这四个长度是教学假设，真实值必须用当前 tokenizer 实测。

token 与 mask 的拼接：

```text
区域:  [ prompt 80 ][ tool_call 38 ][ observation 16 ][ final 9 ]
...
不是总长度 143，也不是 response 区域的所有 token。

### 7.3 四条 rollout 动态 padding

假设同一 prompt 的 4 条采样长度为：

```text
[143, 151, 105, 132]
```

batch 内最大值 `T=151`，短序列右侧补 pad：

```text
input_ids.shape           = [4, 151]
full_response_masks.shape = [4, 151]
full_mask.shape           = [4, 151]
old_per_token_logps.shape = [4, 150]
```

为什么 old logps 少 1？因为长度为 `T` 的序列只有 `T-1` 个“用前面预测后一个 token”的位置。

### 7.4 `full_mask` 和 `completion_mask` 不是一回事

- `full_mask = input_ids != pad_id`：控制 Transformer 不关注 padding。
- `completion_mask = full_response_masks[:, 1:]`：控制哪些预测位置参与 policy loss。

前者是注意力有效性，后者是学习责任范围。把两者混用会造成严重 bug。

---

## 8. Transformer 内部每一步的维度
...
最后一个 `151×151` 表示每个位置对其他位置的相关性。因果 mask 保证位置 `t` 看不到未来 token。
...
```python
logits = res.logits[:, :-1, :]                 # [4, 150, 6400]
target = input_ids[:, 1:]                      # [4, 150]
```
...
```text
log_softmax(logits)             [4, 150, 6400]
target.unsqueeze(-1)            [4, 150, 1]
gather 后                       [4, 150, 1]
squeeze 后 per_token_logps      [4, 150]
```
...
## 11. 为什么每个问题生成 4 条：组相对 Advantage

假设同一 prompt 采样 4 条轨迹：
...
{% raw %}
$$
\sigma=\sqrt{\frac{1}{4}\sum_{i=1}^{4}(R_i-\mu)^2}\approx1.535
$$
{% endraw %}
...
| 轨迹 | 计算 | Advantage |
|---|---|---:|
| `τ1` | `(3.0-0.725)/1.535` | `+1.482` |
| `τ2` | `(1.2-0.725)/1.535` | `+0.309` |
| `τ3` | `(-1.0-0.725)/1.535` | `-1.124` |
| `τ4` | `(-0.3-0.725)/1.535` | `-0.668` |
...
## 12. old policy、current policy、reference policy 各做什么

代码里容易混淆三个概率来源。

### 12.1 old policy：行为策略
...
### 12.2 current policy：正在更新的模型

它产生 `per_token_logps`，有梯度，反向传播最终更新的就是它的参数 `θ`。

### 12.3 reference policy：冻结锚点
...
## 15. 当前 MiniMind 的 CISPO loss：逐符号拆开
...
{% raw %}
$$
\ell=-[1.1\times1.482\times(-0.7)-0.1\times0.02]
\approx1.143
$$
{% endraw %}
...
=-1.1\times1.482=-1.6302
$$
{% endraw %}

梯度下降执行 `参数 ← 参数 - 学习率 × 梯度`。梯度为负，更新倾向于提高这个 token 的 log probability，因此下一次更可能生成它。

若 `A=-1.124`，主项导数变为正数，梯度下降倾向于降低该动作的 log probability。

### 15.4 `<tool_call>` 每个 token 到底算不算 loss
...
## 16. 从 per-token loss 得到一个标量 loss

当前实现不是直接对所有 `N×(T-1)` 位置求平均，而是两层平均。

### 16.1 每条轨迹只平均有效 action token
...
## 19. SFT 与 Agent RL 对同一 tool call 学习方式的根本区别
...
{% raw %}
$$
L_{SFT}=-\frac{1}{C}\sum_t m_t\log\pi_\theta(y_t|y_{<t},x)
$$
{% endraw %}
...
- `80+38+16+9=143`；
- action token 是 `38+9=47`；
- 四个 reward 均值 `0.725`；
- `τ1` advantage 约 `1.482`；
- `logits [4,151,6400]` shift 后 `[4,150,6400]`；
