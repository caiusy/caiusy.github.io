...
updated: 2026-09-13 12:30:00
mathjax: true
description: 为什么缓存 K 和 V，却不缓存 Q；RoPE 为什么作用在 Q、K；MLA 如何只缓存压缩内容和位置 Key。用完整数值例子逐步推导。
categories:
  - AI与大模型
  - 深度学习
tags:
...
cover: /images/kv-cache-rope-mla-explained/pelican-bike.svg
---

这篇文章从一次自回归查询开始，把 KV Cache、RoPE 和 MLA 放到同一条计算链中。每个结论都配有矩阵形状、数值例子与可复现脚本，并回答两个核心问题：为什么历史 Q 不缓存，以及 MLA 缓存之后下一步究竟怎样计算。

<!-- more -->

<div style="margin:1.5rem 0;padding:1rem 1.2rem;border:1px solid #c7d5c9;border-radius:12px;background:#edf2e9">
  <strong>交互阅读版</strong><br>
  <a href="/images/kv-cache-rope-mla-explained/interactive/index.html" target="_blank" rel="noopener">打开带播放、暂停、单步和 RoPE 滑块的完整版本 →</a>
</div>
...
[打开可播放、可暂停的完整交互版](/images/kv-cache-rope-mla-explained/interactive/index.html)。其中有三组可播放、可暂停和逐步推进的动画：缓存追加、二维旋转、MLA 潜空间计算。建议先看正文，再用动画重走一次。

![鹈鹕骑车的小动画：慢慢骑，也慢慢想明白](/images/kv-cache-rope-mla-explained/pelican-bike.svg)
...
![因果注意力只追加一行，历史 KV 被反复读取](/images/kv-cache-rope-mla-explained/cache.svg)
...
![RoPE 的两个绝对旋转在点积中留下相对角度](/images/kv-cache-rope-mla-explained/rope.svg)
...
![原始讲解图：标准 KV Cache 与 MLA 压缩缓存的计算路径对比](/images/kv-cache-rope-mla-explained/kv-cache-vs-mla-original.png)
...
![原始 MLA 架构图：内容 Query、解耦 RoPE、压缩 KV 与推理缓存](/images/kv-cache-rope-mla-explained/mla-decoupled-rope-original.png)
...
![MLA 将内容潜变量和旋转 Key 持久缓存，当前 Query 只用于本步](/images/kv-cache-rope-mla-explained/mla.svg)
...
![KV 缓存从第三步到第四步的完整计算](/images/kv-cache-rope-mla-explained/kv-walkthrough.gif)

![MLA 从潜变量到输出的七步计算](/images/kv-cache-rope-mla-explained/mla-walkthrough.gif)
