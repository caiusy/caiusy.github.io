...
$$ R_{total}(x, y) = r_{RM}(x, y) - \beta \cdot \log \left( \frac{\pi_{\theta}(y \mid x)}{\pi_{ref}(y \mid x)} \right) $$
...
其中 $r_t = \frac{\pi_{new}(a \mid s)}{\pi_{old}(a \mid s)}$ 是概率比率。
...
file_path = "source/_posts/InstructGPT-RLHF-Complete-Guide.md"
